...
`apps/salaries/logic.py` — for `percent` and `per_student` types, salary is based on the sum of `Debt.amount` for all GroupStudents in the group where `Debt.due_date` falls within the billing month being calculated. Debt status (paid/unpaid) does not matter — the teacher did the work regardless of whether the student paid.

- `fixed`:       `fixed_amount` as-is — no debt lookup
- `percent`:     `group_debt_sum × (percent / 100)`
- `per_student`: `group_debt_sum × (per_student_amt / course_price)`

Never use active student count or Payment records for salary calculation.
...
| 9 | Teacher salary (percent/per_student) uses group debt sum (Debt.due_date in billing month), not payments or student count |
