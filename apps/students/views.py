...
class StudentViewSet(ArchiveMixin, CompanyFilterMixin, viewsets.ModelViewSet):
    queryset = Student.objects.select_related('course').annotate(
        status_order=Case(
    ...
    def groups(self, request, pk=None):
        ...
        student = self.get_object()
        qs = GroupStudent.objects.filter(student=student).select_related('group__course', 'group__teacher__user').order_by('-joined_at')
