...
class DebtViewSet(CompanyFilterMixin, mixins.ListModelMixin,
                  mixins.UpdateModelMixin, mixins.RetrieveModelMixin,
                  viewsets.GenericViewSet):
    queryset = Debt.objects.select_related('student').prefetch_related('student__group_memberships__group').order_by('-updated_at')
    ...
    def get_queryset(self):
        ...
        # Support comma-separated status values: ?status=unpaid,overdue
    ...
    def send_sms(self, request, pk=None):
        ...
        if not template:
            return Response({'detail': 'No debt SMS template found.'}, status=status.HTTP_404_NOT_FOUND)

        phone = debt.student.second_phone or debt.student.phone
        if not phone:
            return Response({'detail': 'Student has no phone number.'}, status=status.HTTP_400_BAD_REQUEST)
